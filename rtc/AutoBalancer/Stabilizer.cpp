...
RTC::ReturnCode_t Stabilizer::onDeactivated(RTC::UniqueId ec_id)
{
  ...
  if ( (control_mode == MODE_ST || control_mode == MODE_STAIR) ) {
    sync_2_idle ();
    control_mode = MODE_STIDLE;
  ...
  if (is_legged_robot) {
    ...
    case MODE_ST:
      ...
      if ( transition_count == 0 && !on_ground ) {
          ...
          else control_mode = MODE_STSYNC_TO_AIR;
      } else is_air_counter = 0;
      break;
    case MODE_STSYNC_TO_IDLE:
      sync_2_idle();
      control_mode = MODE_STIDLE;
      break;
    case MODE_STSYNC_TO_AIR:
      sync_2_idle();
      control_mode = MODE_STAIR;
      break;
    }
  }
...
void Stabilizer::getActualParameters ()
{
  ...
  if ( !(control_mode == MODE_STIDLE || control_mode == MODE_STAIR) ) {
    for (size_t i = 0; i < jpe_v.size(); i++) {
      if (is_ik_enable[i]) {
...
void Stabilizer::getTargetParameters ()
{
  ...
  if ( transition_count < 0 ) {
    ...
    if ( transition_count == 1 ) {
      std::cerr << "[" << m_profile.instance_name << "] [" << m_qRef.tm << "] Move to MODE_STIDLE" << std::endl;
  ...
  // Reset prev_ref_cog for transition (MODE_STIDLE=>MODE_ST) because the coordinates for ref_cog differs among st algorithms.
  if (transition_count == (-1 * calcMaxTransitionCount() + 1)) { // max transition count. In MODE_STIDLE => MODE_ST, transition_count is < 0 and upcounter. "+ 1" is upcount at the beginning of this function.
      prev_ref_cog = ref_cog;
      std::cerr << "[" << m_profile.instance_name << "]   Reset prev_ref_cog for transition (MODE_STIDLE=>MODE_ST)." << std::endl;
  }
...
// bool Stabilizer::startStabilizer(void)
// {
//     waitSTTransition(); // Wait until all transition has finished
//     {
//         Guard guard(m_mutex);
//         if ( control_mode == MODE_STIDLE ) {
...
// bool Stabilizer::stopStabilizer(void)
// {
//     waitSTTransition(); // Wait until all transition has finished
//     {
//         Guard guard(m_mutex);
...
//             control_mode = (control_mode == MODE_ST) ? MODE_STSYNC_TO_IDLE : MODE_STIDLE;
//         }
//     }
//     waitSTTransition();
...
// bool Stabilizer::getParameter(OpenHRP::AutoBalancerService::StabilizerParam& i_stp)
// {
...
//   switch(control_mode) {
//   case MODE_STIDLE: i_stp.controller_mode = OpenHRP::AutoBalancerService::MODE_STIDLE; break;
//   case MODE_STAIR: i_stp.controller_mode = OpenHRP::AutoBalancerService::MODE_STAIR; break;
//   case MODE_ST: i_stp.controller_mode = OpenHRP::AutoBalancerService::MODE_ST; break;
//   case MODE_STSYNC_TO_IDLE: i_stp.controller_mode = OpenHRP::AutoBalancerService::MODE_STSYNC_TO_IDLE; break;
//   case MODE_STSYNC_TO_AIR: i_stp.controller_mode = OpenHRP::AutoBalancerService::MODE_STSYNC_TO_AIR; break;
//   default: break;
//   }
...
// bool Stabilizer::setParameter(const OpenHRP::AutoBalancerService::StabilizerParam& i_stp)
// {
//   Guard guard(m_mutex);
...
//       }
//   } else {
//       std::cerr << "[" << m_profile.instance_name << "] cannot change end-effectors except during MODE_STIDLE" << std::endl;
//   }
...
//   } else if (control_mode != MODE_STIDLE) {
...
//   } else {
//     std::cerr << "[" << m_profile.instance_name << "]   st_algorithm cannot be changed to [" << getStabilizerAlgorithmString(st_algorithm) << "] during MODE_STAIR or MODE_ST." << std::endl;
//   }
//   std::cerr << "[" << m_profile.instance_name << "]   emergency_check_mode changed to [" << (emergency_check_mode == OpenHRP::AutoBalancerService::NO_CHECK?"NO_CHECK": (emergency_check_mode == OpenHRP::AutoBalancerService::COP?"COP":"CP") ) << "]" << std::endl;
...
void Stabilizer::waitSTTransition()
{
  ...
  //   2. Check control_mode : Once control_mode is SYNC mode, wait until control_mode moves to the next mode (MODE_STAIR or MODE_STIDLE)
  bool flag = (control_mode == MODE_STSYNC_TO_AIR || control_mode == MODE_STSYNC_TO_IDLE);
  while (transition_count != 0 ||
         (flag ? !(control_mode == MODE_STIDLE || control_mode == MODE_STAIR) : false) ) {
      usleep(10);
      flag = (control_mode == MODE_STSYNC_TO_AIR || control_mode == MODE_STSYNC_TO_IDLE);
  }
  usleep(10);
}
